...
- [ ] T033 Execute `specs/006-frontend-ux-improvements/quickstart.md` §End-to-end smoke walkthrough manually on a fresh checkout. Document any deviations in the PR description. This is the human user-acceptance test for US1/US2/US3 (the per-spec automated tests cover US4). *(Deferred to maintainer post-merge per commit `211bf64` PR description — needs real-browser walkthrough on a clean clone.)*
